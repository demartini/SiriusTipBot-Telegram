        ...
        if name in dir(self.item):
            ...
            if callable(method):
                ...
                raise AttributeError(
                    'Method "%s" is not callable in %s' % (name, self.item)
                )
        else:
...
class SiriusRPC(object):
    def __init__(self, uri, auth):
        self.uri = uri
        self.auth = auth

    def http_post_request(self, name, args):
        data = {"jsonrpc": "1.0", "id": "siriustip", "method": name, "params": args}
        try:
            req = requests.post(
                url=self.uri,
                auth=self.auth,
            ...
            try:
                ...
                del exc_info
                _message = (
                    "Unexpected error occurred. | Name: %s | Args: %s | Traceback:\n%s\n"
                    % (name, args, "".join(_traceback))
                )
...
def main():
    ...
    mySirius = Wrapper(
        SiriusRPC(_config["rpc-uri"], (_config["rpc-user"], _config["rpc-psw"]))
    )
    # getaccountaddress creates an address if account doesn't exist
    res = mySirius.getaddressesbyaccount("tmp")
    if not res["success"]:
    ...
    return
    res = mySirius.sendmany("SiriusTip", {"@demartini": 5, "1234": 6})
    if not res["success"]:
        print("Error: %s" % res["message"])
    else:
        if res["result"]["error"] is not None:
